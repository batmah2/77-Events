...
const Page = () => {
// Utiliser le hook useData() pour obtenir les données actuelles du contexte.
const {data} = useData();

// Utiliser le hook useMemo() pour calculer la valeur de `last` et éviter des calculs inutiles lors des re-rendus.
// useMemo() ne recalculera la valeur que lorsque `data` (qui est passé dans le tableau de dépendance) change.
const last = useMemo(()=>{
  
  // Si `data` est null ou non défini, renvoyer null
  if (!data) return null;

  // Utiliser la fonction de réduction (reduce) sur les événements de `data` pour trouver l'événement le plus récent.
  // `reduce()` passe à travers chaque événement et le compare avec `lastEvent`.
  return data.events?.reduce((lastEvent, event) => {
    
    // Si `lastEvent` est null ou non défini (ce qui sera le cas lors du premier événement), renvoyer l'événement actuel.
    if (!lastEvent) return event;

    // Si la date de l'événement actuel (event.date) est plus récente que la date de `lastEvent`, alors renvoyer l'événement actuel comme le nouvel `lastEvent`.
    if(new Date(lastEvent.date) < new Date(event.date)){
      return event;
    }

    // Si l'événement actuel n'est pas plus récent que `lastEvent`, continuer à renvoyer `lastEvent`.
    return lastEvent;

    // L'événement le plus récent sera conservé et renvoyé après que tous les événements aient été passés en revue.
  })
}, [data]);
